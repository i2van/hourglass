...
/// <summary>
/// A <see cref="SettingsProvider"/> that stores settings in an XML document. In the portable build the document
/// is stored in the same directory as the assembly; otherwise it is stored in the application data directory.
/// </summary>
...
#pragma warning restore S3237
    ...
    internal static string GetSettingsFilePath()
    {
        ...
        string fileName = Path.ChangeExtension(assemblyFileInfo.Name, ".config");
#if PORTABLE
        string directoryName = assemblyFileInfo.DirectoryName ?? ".";
#else
        string applicationName = Path.GetFileNameWithoutExtension(assemblyFileInfo.Name);
        string directoryName = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), applicationName);
#endif
